...
SELECT
    -- === AÑO CURSO ===
    YEAR(CR.FECHA_INICIO_CURSO) AS ANIO_CURSO,
    -- === IDENTIFICACIÓN ===
    ...
    -- === FECHAS DEL CURSO (desde Cursos_Reportev3) ===
    CONVERT(VARCHAR(10), CR.FECHA_INICIO_CURSO, 103) AS FECHA_INICIO_CURSO,
    CONVERT(VARCHAR(10), CR.FECHA_TERMINO_CURSO, 103) AS FECHA_TERMINO_CURSO,
    DATEDIFF(DAY, CR.FECHA_INICIO_CURSO, CR.FECHA_TERMINO_CURSO) AS DIAS_DURACION_CURSO,

    -- ✅ ESTADO DEL RAMO
    CASE
        WHEN CR.FECHA_TERMINO_CURSO < CAST(GETDATE() AS DATE) THEN 'RAMO CERRADO'
        WHEN CR.FECHA_INICIO_CURSO > CAST(GETDATE() AS DATE) THEN 'POR INICIAR'
        ELSE 'RAMO ACTIVO'
    END AS ESTADO_RAMO,

    CASE
        WHEN CR.FECHA_TERMINO_CURSO < CAST(GETDATE() AS DATE)
        THEN DATEDIFF(DAY, CR.FECHA_TERMINO_CURSO, GETDATE())
        ELSE NULL
...
FROM Cursos_Actividades CA

LEFT JOIN Cursos_Reportev3 CR
